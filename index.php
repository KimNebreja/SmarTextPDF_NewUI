    ...
    if (empty($email) || empty($password)) {
        ...
        try {
            ...
            if ($user && password_verify($password, $user['password'])) {
                ...
                // Update last login
                try {
                    error_log("Login process: Attempting to update last_login for user ID: " . $user['user_id']);
                    $updateStmt = $db->prepare("UPDATE users SET last_login = NOW() WHERE user_id = ?");
                    $updateStmt->execute([$user['user_id']]);
                    error_log("Login process: last_login updated successfully.");

                    // Redirect to dashboard only after successful login and update
                    header('Location: pages/dashboard.php');
                    exit;
                } catch (PDOException $e) {
                    error_log("Login Error: Failed to update last_login for user ID " . $user['user_id'] . ": " . $e->getMessage());
                    $error = 'Login successful, but failed to update last login time. Please try again later.';
                }
